...
import type { MessageId, ProfileId, ProjectId, SessionId, WorkspaceId } from "@omniharness/shared-types";
import { type OmniDatabase, openDatabase } from "@omniharness/session-store";
import { afterEach, beforeEach, describe, expect, it } from "vitest";
import { importPiSession } from "./pi-session.js";

export function seedDb(): { db: OmniDatabase; profileId: ProfileId; projectId: ProjectId; workspaceId: WorkspaceId } {
  ...
  db.profiles.put({ id: profileId, name: "Test", isDefault: true, createdAt: new Date().toISOString() });
...
describe("importPiSession", () => {
  it("imports a session with branches and preserves the tree", () => {
    const path = writeJsonl("session.jsonl", [
      HEADER,
      { type: "session_info", id: "a0", parentId: null, timestamp: "2026-07-01T10:00:01.000Z", name: "My chat" },
      { type: "message", id: "a1", parentId: "a0", timestamp: "2026-07-01T10:00:02.000Z",
        message: { role: "user", content: "hello", timestamp: 1 } },
      { type: "message", id: "b1", parentId: "a1", timestamp: "2026-07-01T10:00:03.000Z",
        message: { role: "assistant", content: [{ type: "text", text: "hi" }, { type: "thinking", thinking: "hmm" }],
          provider: "anthropic", model: "claude", api: "anthropic-messages",
          usage: { input: 5, output: 3, cacheRead: 1, cacheWrite: 0, totalTokens: 9, cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0.01 } },
          stopReason: "stop", timestamp: 2 } },
      // Branch: two children of a1.
      { type: "message", id: "c1", parentId: "a1", timestamp: "2026-07-01T10:00:04.000Z",
        message: { role: "user", content: "branch two", timestamp: 3 } },
      { type: "message", id: "d1", parentId: "b1", timestamp: "2026-07-01T10:00:05.000Z",
        message: { role: "toolResult", toolCallId: "call_1", toolName: "bash", content: [{ type: "text", text: "ok" }], isError: false, timestamp: 4 } },
      { type: "model_change", id: "e1", parentId: "d1", timestamp: "2026-07-01T10:00:06.000Z", provider: "openai", modelId: "gpt-5" },
      { type: "compaction", id: "f1", parentId: "e1", timestamp: "2026-07-01T10:00:07.000Z", summary: "so far...", firstKeptEntryId: "c1", tokensBefore: 12345 },
      { type: "custom_message", id: "g1", parentId: "f1", timestamp: "2026-07-01T10:00:08.000Z", customType: "my-ext", content: "injected", display: true },
      { type: "weird_ext_entry", id: "h1", parentId: "g1", timestamp: "2026-07-01T10:00:09.000Z", payload: { x: 1 } },
    ]);
    ...
    expect(branches.map((m) => m.id).sort()).toEqual(["msg_pi_sess-uuid-1_b1", "msg_pi_sess-uuid-1_c1"]);
    ...
    expect(db.messages.get("msg_pi_sess-uuid-1_f1" as MessageId)!.parts[0]?.text).toContain("[pi:compaction");
  ...
  it("is idempotent: re-import skips the session", () => {
    const path = writeJsonl("s.jsonl", [
      HEADER,
      { type: "message", id: "a1", parentId: null, timestamp: "2026-07-01T10:00:02.000Z",
    ...
    expect(second.skipped).toEqual([{ id: "sess-uuid-1", reason: "already imported as sess_pi_sess-uuid-1" }]);
