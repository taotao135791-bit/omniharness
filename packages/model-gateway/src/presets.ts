...
export const PROVIDER_PRESETS: readonly ProviderPreset[] = [
  ...
  },
  {
    ...
    openAiCompatible: true,
    implemented: true,
    note: "Local server; no API key required.",
  },
  {
    id: "lmstudio",
    kind: "lmstudio",
...
export function resolvePreset(id: string, options?: AzureOpenAiPresetOptions): ResolvedPreset {
  ...
  if (preset === undefined) {
    throw new Error(
      `Unknown provider preset "${id}". Known presets: ${PROVIDER_PRESETS.map((p) => p.id).join(", ")}`,
    );
  }
  ...
  if (preset.kind === "azure-openai") {
    if (options === undefined || options.resource === "" || options.deployment === "") {
      throw new Error(
        "The azure-openai preset requires options: { resource, deployment, apiVersion? }",
      );
    }
